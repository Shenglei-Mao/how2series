...
function EvaluationOnce(client) {
    try {
        console.log("-----------一次性评估示例------------")
        ...
        let initRsp = await
        InitOralProcess(client, initReq);
        console.log("初始化返回结果：")
        ...
        let transRsp = await
        TransmitOralProcess(client, transReq)
        console.log("评估结果：")
...
function EvaluationMultipleSeq(client) {
...
function EvaluationMultipleSeqDisorder(client) {
...
function EvaluationAsync(client) {
    try {
        console.log("----------异步评估示例------------")
        let sessionId = uuid.v1();
        // 初始化评估
...
function EvaluationTransmitOralProcessWithInit(client) {
    ...
    try {
        ...
        for (let i = 1; i <= pkgNum; i++) {
            ...
            transReq.UserVoiceData = base64Str;
            let transRsp = await
            TransmitOralProcessWithInit(client, transReq)
            console.log("评估结果：")
            console.log(transRsp.to_json_string())
            let costTime = Date.now() - startTime
            console.log("完成评估耗时:" + costTime)
